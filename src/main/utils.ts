...
export function parsePaperSize(pageSize?: PaperSize | SizeOptions): { width: number, height: number } {
    ...
    if (typeof pageSize == 'string') {
        ...
        width = convertMillimetersToPixels(pageSize.width);
        height = convertMillimetersToPixels(pageSize.height);
    }
...
export function parsePaperSizeInMicrons(pageSize?: PaperSize | SizeOptions): { width: number, height: number } {
    ...
    if (typeof pageSize == 'string') {
        ...
        width = convertMillimetersToMicrons(pageSize.width);
        height = convertMillimetersToMicrons(pageSize.height);
    }
...
export function convertMillimetersToPixels(millimeters: number): number {
    return millimeters * (96 / 25.4)
}

export function convertMillimetersToMicrons(millimeters: number): number {
